// REMOVIDO: Slider Switch funcionalidade (conflito com index.js)
// Mantido apenas em assets/js/index.js para única fonte da verdade
/*
...
(() => {
  ...
  const lic = (() => {
    ...
    const set = (on) => {
      ...
      setTimeout(() => sw.classList.remove('pulse'), 450);
    };
  ...
  function updateVisibility() {
    ...
    if (showLic)
      lic.set('nao'); // reforça reset ao exibir
  ...
  async function abrirWizardHtml(url) {
    ...
    openWizardModal();
    els.wizInner.innerHTML =
      '<div style="padding:32px;text-align:center;color:#666">Carregando…</div>';
    try {
      ...
      temp.innerHTML = html;
      const main =
        temp.querySelector('main, .wizard-financeiro-container, .wizard-container') || temp;
      els.wizInner.innerHTML = '';
      els.wizInner.appendChild(main.cloneNode(true));
      // Para wizard-juridica.html, apenas inicializa se já carregado (evita duplicidade)
          ...
          if (stepsDiv && contentDiv) {
            if (typeof window.initWizardJuridica === 'function') {
              try {
                ...
                console.debug('initWizardJuridica falhou:', e);
              }
            }
          } else if (tries < 10) {
            tries++;
      ...
      els.wizInner.innerHTML =
        '<div style="padding:48px;text-align:center;color:#c00;">Erro ao carregar o wizard: ' +
        err.message +
        '</div>';
    }
  }
  ...
  window.acaoIniciarWizard = function acaoIniciarWizard() {
    ...
    const acao = els.contratar?.value;
    const tipo = els.formalizar?.value;
    // const tipoContr = els.tipoContr?.value; // não utilizado no fluxo atual
  ...
  async function buildDocumentationList() {
    ...
    if (!container) return;
    container.innerHTML =
      '<div style="padding:24px;text-align:center;color:#666;">Carregando documentação…</div>';
    try {
      const docUrl = window.withBase ? window.withBase('documentacao.json') : 'documentacao.json';
      ...
      buckets.forEach((cat) => {
        ...
        if (cat.titulo && (cat.descricao || cat.url) && !cat.documentos) {
          ...
          li.style.cssText =
            'padding:10px 12px; border:1px solid #e7e7e7; border-radius:10px; background:#fff;';
      ...
      container.innerHTML =
        '<div style="padding:48px;text-align:center;color:#c00;">Erro ao carregar a documentação: ' +
        err.message +
        '</div>';
    }
  }
  ...
  function openDocModal() {
    if (els.docOverlay) {
      els.docOverlay.style.display = 'flex';
    }
  }
  function closeDocModal() {
    if (els.docOverlay) {
      els.docOverlay.style.display = 'none';
      if (els.docInner) {
        els.docInner.innerHTML = '';
      }
    }
  }
  on(els.docOverlay, 'click', (e) => {
    if (e.target === els.docOverlay) closeDocModal();
  });
  ...
  function buscarImoveis() {
    ...
    setTimeout(() => {
      filteredImoveis = mockImoveis.filter((imovel) => {
        ...
        const matchSearch =
          !s ||
          imovel.codigo.toLowerCase().includes(s) ||
          imovel.denominacao.toLowerCase().includes(s) ||
          imovel.local.toLowerCase().includes(s);
  ...
  function updatePaginationInfo() {
    ...
    const el = $('#pagination-info');
    if (el) el.textContent = `Página ${currentPage} de ${total}`;
  ...
  function changeItemsPerPage() {
    const sel = $('#items-per-page');
    if (!sel) return;
    itemsPerPage = parseInt(sel.value || '10', 10);
    currentPage = 1;
    renderResultados();
  }
  function limparFiltros() {
    const s = $('#search-imovel'),
      f = $('#status-filter');
    if (s) s.value = '';
    if (f) f.value = '';
    mostrarEstruturaTabela();
  ...
  on(document, 'click', (e) => {
    ...
    if (cmd === 'sel') {
      const item =
        filteredImoveis.find((i) => i.codigo === id) || mockImoveis.find((i) => i.codigo === id);
  ...
  function iniciarSolicitacao() {
    ...
    if (contratar === 'contratar' && !tipoContr)
      return alert('Por favor, selecione o tipo de contratação.');
    if (contratar === 'regularizar' && !valor) return alert('Por favor, informe o valor.');
    if (contratar === 'formalizar' && !atoFormal)
      return alert('Por favor, selecione o ato formal.');
  ...
  function initWizardFinanceiro() {
    ...
    on(addBtn, 'click', () => {
      const container = $('#dados-bancarios-container');
      if (!container) return;
      const novo = document.createElement('div');
      novo.className = 'banco-block';
      ...
      requestAnimationFrame(() => {
        novo.style.transition = 'all .3s ease';
  ...
  function validarFormularioFinanceiro() {
    ...
    agencias.forEach((a) => {
      a.style.borderColor = a.value.trim() ? '#ddd' : '#dc3545';
      if (!a.value.trim()) ok = false;
    });
    contas.forEach((c) => {
      c.style.borderColor = c.value.trim() ? '#ddd' : '#dc3545';
      if (!c.value.trim()) ok = false;
    });
    return ok;
  }
  function coletarDadosFormularioFinanceiro() {
    ...
    $$('.banco-block').forEach((b, i) => {
      ...
      if (agencia && conta)
        locadores.push({ id: i + 1, agencia, agenciaNome, conta, tipoConta: tipo });
    });
    return {
  ...
  function removerLocador(btn) {
    const block = btn.closest('.banco-block');
    if (!block) return;
