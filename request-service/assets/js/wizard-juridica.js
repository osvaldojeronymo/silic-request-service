(() => {
  'use strict';
  // evita rodar duas vezes (HMR/dev, duplicidade de <script>, etc.)
  if (window.__WIZARD_JURIDICA_INIT__) return;
  window.__WIZARD_JURIDICA_INIT__ = true;

  // helper de readiness para evitar tocar DOM antes da hora
  const onReady = (fn) =>
    document.readyState === 'loading'
      ? document.addEventListener('DOMContentLoaded', fn, { once: true })
      : fn();

  // expõe a função no escopo global para ser utilizada via carregamento dinâmico
  window.initWizardJuridica = function initWizardJuridica() {
    console.log('=== INÍCIO INIT WIZARD ===');
    
    // containers obrigatórios do wizard (presentes no HTML do wizard)
    const stepsHost = document.getElementById('wizardSteps');
    const contentHost = document.getElementById('wizardContent');
    
    console.log('Elementos encontrados:');
    console.log('- stepsHost:', !!stepsHost);
    console.log('- contentHost:', !!contentHost);
    
    if (!stepsHost || !contentHost) {
      console.error('Elementos wizardSteps ou wizardContent não encontrados!');
      return; // nada a fazer se o HTML não estiver presente
    }

    // Verifica se deve incluir step de licitação
    const params = new URLSearchParams(location.search);
    const comLicitacao = params.get('licitacao') === 'sim';
    
    console.log('=== DEBUG WIZARD ===');
    console.log('URL atual:', location.href);
    console.log('Parâmetros URL:', params.toString());
    console.log('Parâmetro licitacao:', params.get('licitacao'));
    console.log('Com licitação?', comLicitacao);
    
    const steps = [];
    if (comLicitacao) {
      console.log('Adicionando step de Licitação...');
      steps.push({
        label: 'Licitação',
    ...
    console.log('Adicionando demais steps...');
    steps.push(
      {
        label: 'Documentação',
    ...
    function renderWizard() {
      ...
      steps.forEach((step, idx) => {
        ...
        stepDiv.className =
          'wizard-step' + (idx === currentStep ? ' active' : idx < currentStep ? ' completed' : '');
      ...
      if (currentStep === 0) {
        ...
        const cancel = document.getElementById('btnCancelWizard');
        if (cancel)
        ...
        const prev = document.getElementById('btnPrevStepDoc');
        if (prev)
          prev.onclick = function () {
            currentStep--;
            renderWizard();
          };
        const next = document.getElementById('btnNextStepDoc');
        if (next)
          next.onclick = function () {
            currentStep++;
            renderWizard();
        ...
        fetch(
          ...
          .then((data) => {
            ...
            if (secaoA) {
              ...
              orientacaoA.textContent =
                'Por favor, insira a documentação que justifica a sua solicitação.';
            ...
            if (secaoB) {
              ...
              orientacaoB.textContent =
                'Por favor, insira a documentação pertinente à avaliação do imóvel.';
        ...
        // Passo 3 - Compliance
        const prev = document.getElementById('btnPrevStepCompliance');
        if (prev)
          prev.onclick = function () {
            currentStep--;
            renderWizard();
          };
        const next = document.getElementById('btnNextStepCompliance');
        if (next)
          next.onclick = function () {
            currentStep++;
            renderWizard();
        ...
        // Passo 4 - Jurídico
        const prev = document.getElementById('btnPrevStepJuridico');
        if (prev)
          prev.onclick = function () {
            currentStep--;
            renderWizard();
          };
        const next = document.getElementById('btnNextStepJuridico');
        if (next)
          next.onclick = function () {
            currentStep++;
            renderWizard();
        ...
        // Passo 5 - Financeiro
        const prev = document.getElementById('btnPrevStepFinanceiro');
        if (prev)
          prev.onclick = function () {
            currentStep--;
            renderWizard();
          };
        const next = document.getElementById('btnNextStepFinanceiro');
        if (next)
          next.onclick = function () {
            currentStep++;
            renderWizard();
        ...
        // Passo 6 - Confirmação
        const nova = document.getElementById('btnNovaSolicitacao');
        if (nova)
          nova.onclick = function () {
            currentStep = 0;
            renderWizard();
          };
        const encerrar = document.getElementById('btnEncerrar');
        if (encerrar)
  ...
  // opcional: se o HTML do wizard já estiver na página, inicializa após DOM pronto
  onReady(() => {
    const stepsHost = document.getElementById('wizardSteps');
    const contentHost = document.getElementById('wizardContent');
    if (stepsHost && contentHost && typeof window.initWizardJuridica === 'function') {
      try {
        window.initWizardJuridica();
      } catch (e) {
        // ignora erros de auto-init
        if (typeof console !== 'undefined' && typeof console.debug === 'function') {
          console.debug('[wizard-juridica] auto-init falhou', e);
        }
      }
    }
  });
