...
# --- Coleta robusta de caminhos (suporta espaços/acentos) ---
...
# --- 1) CSS: remover @import remoto e anexar melhorias ---
CSS_APPEND='
...
# --- 2) index.html: remover include direto de wizard-juridica.js ---
for ih in "${INDEX_HTMLS[@]}"; do
  ...
  if [ "$after" != "$before" ]; then
    changed+=("Removed direct wizard-juridica.js include -> $ih")
  fi
done

# --- 3) wizard HTMLs: garantir data-wizard-action nos botões ---
for wh in "${WIZ_HTMLS[@]}"; do
  ...
  if [ "$content" != "$orig" ]; then
    printf "%s" "$content" > "$wh"
    changed+=("Added data-wizard-action -> $wh")
  fi
done

# --- 4) index.js: sincronizar switch Licitação e lazy-load do wizard ---
PATCH_SNIPPET='
...
(function(){
  try {
    ...
    if (licSwitch){
      ...
      licSwitch.addEventListener("change", () => {
        window._licitacaoSim = !!licSwitch.checked;
      });
    }
...
PATCH
chmod +x patch_wizard_v2.sh
echo "Script salvo como patch_wizard_v2.sh"
