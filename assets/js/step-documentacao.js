  ...
  checklist.sections.forEach((section) => {
    ...
    section.items.forEach((item) => {
      ...
      fileInput.addEventListener('change', e => {
        ...
        fileInfo.textContent = respostas[item.id].arquivo ? `Arquivo: ${respostas[item.id].arquivo.name}` : '';
      });
...
document.addEventListener('DOMContentLoaded', function() {
  renderChecklist('root', 'assets/data/checklists/documentacao-pj_nova_unidade.json');
});
...
export async function renderStepDocumentacao(container, variantKey) {
  const data = (await loadChecklistDoc(`/assets/data/checklists/documentacao-pj_nova_unidade.json`))[0];
  container.innerHTML = '';
  // Pergunta condicional para crédito em favor de terceiro
  const perguntaDiv = document.createElement('div');
  perguntaDiv.style = 'margin-bottom:18px;padding:12px 18px;background:#f5f7fa;border-radius:8px;font-size:1.08rem;color:#003366;';
